...
EOF`
	...
	// Test that NOTICE prefixes are stripped
	if config.Global["pid"] != "/var/run/php-fpm.pid" {
		t.Errorf("Expected pid to be '/var/run/php-fpm.pid', got '%s'", config.Global["pid"])
	}
...
func TestGetPoolConfigMetrics(t *testing.T) {
	tests := []struct {
		name     string
		pool     map[string]string
		expected PoolConfigMetrics
	}{
		{
			name: "dynamic pool",
			pool: map[string]string{
			...
			expected: PoolConfigMetrics{
				...
				MaxSpareServers:         10,
				MaxRequests:             500,
				ProcessIdleTimeout:      10, // default
				RlimitFiles:             2048,
				RlimitCore:              0, // "unlimited" not parseable as int
		...
		},
		{
			...
			expected: PoolConfigMetrics{
				...
				MinSpareServers:         1,    // default
				MaxSpareServers:         3,    // default
				MaxRequests:             0,    // default
				ProcessIdleTimeout:      10,   // default
				RlimitFiles:             1024, // default
				RlimitCore:              0,    // default
				RequestSlowlogTimeout:   0,    // default
				RequestTerminateTimeout: 0,    // default
			},
		},
		{
			name: "ondemand pool",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := GetPoolConfigMetrics(tt.pool)

			if result.PM != tt.expected.PM {
				t.Errorf("PM: expected %s, got %s", tt.expected.PM, result.PM)
			}
			if result.MaxChildren != tt.expected.MaxChildren {
				t.Errorf("MaxChildren: expected %d, got %d", tt.expected.MaxChildren, result.MaxChildren)
			}
			if result.StartServers != tt.expected.StartServers {
				t.Errorf("StartServers: expected %d, got %d", tt.expected.StartServers, result.StartServers)
			}
			if result.MinSpareServers != tt.expected.MinSpareServers {
				t.Errorf("MinSpareServers: expected %d, got %d", tt.expected.MinSpareServers, result.MinSpareServers)
			}
			if result.MaxSpareServers != tt.expected.MaxSpareServers {
				t.Errorf("MaxSpareServers: expected %d, got %d", tt.expected.MaxSpareServers, result.MaxSpareServers)
			}
			if result.MaxRequests != tt.expected.MaxRequests {
				t.Errorf("MaxRequests: expected %d, got %d", tt.expected.MaxRequests, result.MaxRequests)
			}
			if result.ProcessIdleTimeout != tt.expected.ProcessIdleTimeout {
				t.Errorf("ProcessIdleTimeout: expected %d, got %d", tt.expected.ProcessIdleTimeout, result.ProcessIdleTimeout)
			}
			if result.RlimitFiles != tt.expected.RlimitFiles {
				t.Errorf("RlimitFiles: expected %d, got %d", tt.expected.RlimitFiles, result.RlimitFiles)
			}
			if result.RlimitCore != tt.expected.RlimitCore {
				t.Errorf("RlimitCore: expected %d, got %d", tt.expected.RlimitCore, result.RlimitCore)
			}
			if result.RequestSlowlogTimeout != tt.expected.RequestSlowlogTimeout {
				t.Errorf("RequestSlowlogTimeout: expected %d, got %d", tt.expected.RequestSlowlogTimeout, result.RequestSlowlogTimeout)
			}
			if result.RequestTerminateTimeout != tt.expected.RequestTerminateTimeout {
				t.Errorf("RequestTerminateTimeout: expected %d, got %d", tt.expected.RequestTerminateTimeout, result.RequestTerminateTimeout)
			}
		})
	}
}
